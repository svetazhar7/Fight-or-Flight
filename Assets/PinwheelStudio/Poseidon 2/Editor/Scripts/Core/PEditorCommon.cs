...
namespace Pinwheel.Poseidon
{
    public static class PEditorCommon
    {
        ...
        public static T ObjectSelectorDragDrop<T>(Rect r, string message, string filter = "") where T : class
        {
                ...
                e.type == EventType.Repaint))
            {
                    ...
                    draggedObject[0] is T)
                {
                    ...
                    EditorGUI.DrawRect(r, Utilities.GetColor(PEditorCommon.selectedItemColor, 0.5f));
        ...
        public static void DrawPlanarReflectionHelper(PoseidonWaterBody water)
        {
            if (water.material != null)
            {
                bool hasReflectionEffect = PMat.HasReflectionFX(water.material);
                if (hasReflectionEffect)
                {
                    PEditorCommon.Foldout(PRGUI.HEADER, true, PRGUI.ID, () =>
                    {
                        PlanarReflectionRenderer reflectionRenderer = water.GetComponent<PlanarReflectionRenderer>();
                        if (reflectionRenderer == null)
                        {
                            ...
                            {
                                Undo.AddComponent<PlanarReflectionRenderer>(water.gameObject);
        ...
        public static void DrawWaveMaskHelper(PoseidonWaterBody water)
        {
            if (water.material != null)
            {
                bool hasWaveMaskEffect = PMat.HasWaveMaskFX(water.material);
                if (hasWaveMaskEffect)
                {
                    PEditorCommon.Foldout(WMGUI.HEADER, true, WMGUI.ID, () =>
                    {
                        WaveMaskController controller = water.GetComponent<WaveMaskController>();
                        if (controller == null)
                        {
                            EditorGUILayout.LabelField($"You need to add a {nameof(WaveMaskController)} component for controlling world space wave mask.");
                            if (GUILayout.Button("+Add"))
                            {
                                Undo.AddComponent<WaveMaskController>(water.gameObject);
                                EditorUtility.SetDirty(water.gameObject);
                            }
                        }
                        else
        ...
        public static T FindFirstAssetWithLabel<T>(string label) where T : Object
        {
