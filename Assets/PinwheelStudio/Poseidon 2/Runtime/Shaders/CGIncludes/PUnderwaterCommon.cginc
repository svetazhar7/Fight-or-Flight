...
float2 DistorUV(float2 uv)
{
	...
	float4 n0 = PSAMPLE_TEXTURE2D(_DistortionTex, uv + flow * _Time.y * 0.025);
	float4 n1 = PSAMPLE_TEXTURE2D(_DistortionTex, uv * 2 - flow * _Time.y * 2 * 0.025);
	float3 n = (_UnpackNormal(n0) + _UnpackNormal(n1)) * 0.5;
	n *= _DistortionStrength * 0.025;

	uv -= float2(n.x, n.y) * fade;
	return uv;
}
#endif
