    ...
    public class PropertiesView : VisualElement
    {
        ...
        private class ExploreGUI
        {
            ...
            public static void DrawNewsEntry(EditorSettings.MarketingSettings.NewsEntry e)
            {
                ...
                EditorGUILayout.LabelField(e.title, EditorStyles.label, GUILayout.ExpandWidth(false));
                ...
                if (Event.current.type == EventType.MouseDown && entryRect.Contains(Event.current.mousePosition) && !string.IsNullOrEmpty(e.link))
                {
                    ...
                    NetUtils.TrackClick("news-entry", UILocation.GraphEditor_Noti);
                }
            ...
            public static void DrawAssetEntry(EditorSettings.MarketingSettings.AssetEntry e)
            {
                ...
                string title = $"{e.name} <i><color=orange>{e.promotionText}</color></i>";
                ...
                if (Event.current.type == EventType.MouseDown && entryRect.Contains(Event.current.mousePosition) && !string.IsNullOrEmpty(e.link))
                {
                    string url = e.link;
                    Application.OpenURL(url);
                    NetUtils.TrackClick("asset-entry", UILocation.GraphEditor_Noti);
                }
            }
        ...
        public static void OnExploreGUI()
        {
            ...
            if (news.Length > 0)
            {
                EditorGUILayout.LabelField(ExploreGUI.NEWS, EditorStyles.boldLabel);
            ...
            //{
            //    EditorGUILayout.LabelField(ExploreGUI.OTHER_PRODUCTS_FROM_PINWHEEL, EditorStyles.boldLabel);
