...
namespace Pinwheel.VistaEditor.Graph
{
    public class GraphEditorGraphView : GraphView
    {
        ...
        // Carries one Unity asset reference through the clipboard JSON.
        // UnityEngine.Object can't be stored in JSON, so each ref is stored as its AssetDatabase identity
        // and resolved back on paste. Lives in SerializeData so it survives domain reloads and cross-graph paste.
        [System.Serializable]
        public struct AssetRefCopyPaste
        {
            public string key;     // "nodeId::fieldName" — matches GraphAsset.GetObjectSerializeKey
            public string guid;    // AssetDatabase GUID of the asset file
            public long localId;   // local file ID — needed to identify sub-assets within a file
        }
        ...
        public struct SerializeData
        {
            ...
            [SerializeField]
            public List<AssetRefCopyPaste> m_assetRefs;
            ...
            public static SerializeData FromJson(string json)
            {
                ...
                data.m_assetRefs = new List<AssetRefCopyPaste>();
        ...
        private IEnumerator OnNodeSearcherItemSelectedDelayed(SearcherItem item, NodeCreationContext context)
        {
            ...
            else if (string.Equals(itemData.hint, SearcherItemData.HINT_PLACEHOLDER))
            {
                Vector2 mouseWorldPos = context.screenMousePosition - m_editor.position.position; ;
                Rect activatorRect = new Rect(mouseWorldPos, Vector2.one);
                SearcherCandidatePlaceholder placeholder = Activator.CreateInstance(itemData.nodeType) as SearcherCandidatePlaceholder;
                placeholder.OnCommit(activatorRect);
            }
            else
            {
                Vector2 mouseWorldPos = context.screenMousePosition - m_editor.position.position; ;
        ...
        private string OnSerializeElements(IEnumerable<GraphElement> elements)
        {
            ...
            List<ObjectReference> m_assetRefs = new List<ObjectReference>();
            ...
            foreach (ObjectReference oRef in m_assetRefs)
            {
                if (AssetDatabase.TryGetGUIDAndLocalFileIdentifier(oRef.target, out string guid, out long localId))
                {
                    data.m_assetRefs.Add(new AssetRefCopyPaste { key = oRef.key, guid = guid, localId = localId });
                }
            }
        ...
        private void OnUnserializeAndPasteElements(string operationName, string json)
        {
            ...
            // Resolve GUID-based refs from the clipboard JSON into live ObjectReferences.
            // This replaces the old m_assetRefs field approach, which broke after domain reloads
            // and when pasting into a different graph view.
            List<ObjectReference> resolvedRefs = new List<ObjectReference>();
            foreach (AssetRefCopyPaste entry in data.m_assetRefs)
            {
                string path = AssetDatabase.GUIDToAssetPath(entry.guid);
                if (string.IsNullOrEmpty(path))
                    continue;
                UnityEngine.Object asset = AssetDatabase.LoadAllAssetsAtPath(path)
                    .FirstOrDefault(a => AssetDatabase.TryGetGUIDAndLocalFileIdentifier(a, out _, out long lid) && lid == entry.localId);
                if (asset == null)
                    asset = AssetDatabase.LoadMainAssetAtPath(path);
                if (asset != null)
                    resolvedRefs.Add(new ObjectReference(entry.key, asset));
            }

            List<INode> nodes = Serializer.Deserialize<INode>(data.m_nodes);
            foreach (INode n in nodes)
            {
                GraphAsset.DeserializeAssetReferences(resolvedRefs, n);
