...
namespace Pinwheel.VistaEditor.Graph
{
    public class GraphEditorBase : EditorWindow
    {
        ...
        public static GraphEditorBase OpenGraph(GraphAsset graph, IExternalInputProvider inputProvider = null)
        {
            ...
            if (graph is BiomeMaskGraph bmg)
            {
                switch (uiVersion)
                {
                    case GraphEditorUIVersion.V2:
                        return GraphEditorBase.OpenGraph<BiomeMaskGraphEditor2>(bmg, inputProvider);
                    default:
                        return GraphEditorBase.OpenGraph<BiomeMaskGraphEditor>(bmg, inputProvider);
                }
            }
            else if (graph is TerrainGraph tg)
            {
                switch (uiVersion)
                {
                    case GraphEditorUIVersion.V2:
