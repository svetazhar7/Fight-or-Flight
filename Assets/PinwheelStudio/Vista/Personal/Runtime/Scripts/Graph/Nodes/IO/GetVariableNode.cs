    ...
    public class GetVariableNode : ExecutableNodeBase, IHasDependencyNodes, ISerializationCallbackReceiver, IHasDynamicSlotCount
    {
        ...
        public override void ExecuteImmediate(GraphContext context)
        {
            if (m_outputSlot != null)
            {
                ...
                if (context.IsTargetNode(m_id) && !string.IsNullOrEmpty(m_varName))
                {
                    SlotRef sourceRef = context.GetVariable(m_varName);
                    if (sourceRef.Equals(SlotRef.invalid))
                        return;

                    if (m_outputSlot is BufferSlot)
                    {
                        ComputeBuffer sourceBuffer = context.GetBuffer(sourceRef);
                    ...
                    {
                        RenderTexture sourceTexture = context.GetTexture(sourceRef);
