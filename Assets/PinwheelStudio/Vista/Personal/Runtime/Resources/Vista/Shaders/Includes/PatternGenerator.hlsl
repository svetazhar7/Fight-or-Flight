	...
	float _uintToFloat01(uint value)
	{
		return value * (1.0 / 4294967295.0);
	}

	float3 _openSimplexSeedOffset(int seed)
	{
		uint s = (uint)seed;
		uint hx;
		uint hy;
		...
		_Hash_Tchou_2_1_uint(uint2(s, 0x68bc21ebu), hx);
		_Hash_Tchou_2_1_uint(uint2(s ^ 0x02e5be93u, 0x967a889bu), hy);
		_Hash_Tchou_2_1_uint(uint2(s ^ 0x85ebca6bu, 0xc2b2ae35u), hz);

		return float3(hx, hy, hz) * (251.0 / 4294967295.0);
	}

	float openSimplexNoise(float2 uv, int seed)
	{
		// OpenSimplex2 rotates XY and Z together, so very large Z slices can drown out XY variation.
		// Keep the existing direct-slice behavior for normal seed ranges, but compact very large seeds
		// into a bounded deterministic offset to preserve variation for any 32-bit seed.
		float3 coord;
		if (seed >= -1048576 && seed <= 1048576)
		{
			coord = float3(uv, seed);
		}
		else
		{
			float3 seedOffset = _openSimplexSeedOffset(seed);
			coord = float3(uv + seedOffset.xy, seedOffset.z);
		}

		float4 result = openSimplex2_ImproveXY(coord);
		return result.w;
	}
#endif
